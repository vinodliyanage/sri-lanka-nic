...
```typescript
import type { NICInstance, NICConfig, NICValidatorConfig, ResolvedNICConfig, NICOptions, NICType, Gender, Birthday, SafeParseResult } from "@sri-lanka/nic";
```
...
A discriminated union returned by `NIC.safeParse()`:

```typescript
type SafeParseResult =
  | { success: true; data: NICInstance }
  | { success: false; error: Error };
```

Use the `success` flag to narrow the type:

```typescript
const result = NIC.safeParse("901404567V");

if (result.success) {
  result.data.gender; // ✅ NICInstance
} else {
